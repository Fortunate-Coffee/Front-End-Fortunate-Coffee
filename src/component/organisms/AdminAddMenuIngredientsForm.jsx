...
const AdminAddMenuIngredientsForm = ({ menuId, setShowAddMenuIngredientsForm }) => {
    const [ingredients, setIngredients] = useState([]);
    const [warning, setWarning] = useState("");
    const [success, setSuccess] = useState("");
    const [loading, setLoading] = useState(false);
    const [formDataList, setFormDataList] = useState([
        { food_ingredients_id: '', menu_ingredients_qty: '' }
    ]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            console.error('Error adding menu ingredients:', error);
        } finally {
                        ...
                        <div key={index} className="flex flex-row justify-between my-1">
                            <div className="flex items-center">
                                ...
                                <select id={`ingredient-${index}`} name="food_ingredients_id" value={formData.food_ingredients_id} onChange={(e) => handleChange(e, index)} required className="mt-1 p-2 border border-gray-300 rounded-md w-full shadow-xl">
                                    ...
                                    {getAvailableIngredients(index).map((ingredient, i) => (
                                        <option key={i} value={ingredient.food_ingredients_id}>{ingredient.food_ingredients_name}</option>
                                    ))}
                                </select>
                            </div>
...
export default AdminAddMenuIngredientsForm;
